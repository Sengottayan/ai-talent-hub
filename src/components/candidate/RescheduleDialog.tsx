...
export function RescheduleDialog({ open, onOpenChange, interview, candidateId, onSuccess }: RescheduleDialogProps) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            toast({
                ...
                description: "Your reschedule request has been sent to HR for review. You will be notified once it is approved or rejected.",
            });
            ...
            if (error?.response?.status === 409 || msg.toLowerCase().includes("already") || msg.toLowerCase().includes("pending")) {
                setAlreadyPending(true);
                toast({
                    title: "⚠️ Request Already Pending",
                    description: msg || "You already have an active reschedule request for this interview. Please wait for HR to review it.",
                    variant: "destructive",
                });
            } else {
                toast({
    ...
    return (
        <Dialog open={open} onOpenChange={(v) => { onOpenChange(v); if (!v) setAlreadyPending(false); }}>
            <DialogContent className="sm:max-w-[460px]">
                <DialogHeader>
                    <DialogTitle>Request Interview Reschedule</DialogTitle>
                    <DialogDescription>
                        Propose a new <strong>date and time</strong> for your{" "}
                        <span className="font-semibold text-foreground">{interview?.jobRole}</span> interview.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="grid gap-5 py-2">
                    ...
                    <div className="grid gap-2">
                        <Label htmlFor="time" className="flex items-center gap-1.5">
                            ...
                            Preferred Time <span className="text-xs font-normal text-muted-foreground ml-1">(business hours · IST)</span>
                        </Label>
                        ...
                        {timeSlot && (
                            <p className="text-xs text-muted-foreground">
                                Selected: <strong>{TIME_SLOTS.find(s => s.value === timeSlot)?.label}</strong>
                                {date && <> on <strong>{format(date, "EEE, MMM d")}</strong></>}
                            </p>
                        )}
                    </div>
