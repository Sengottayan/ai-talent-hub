...
const TimerComponent: React.FC<TimerComponentProps> = ({
  ...
  useEffect(() => {
    ...
    if (minutes && minutes > 0) {
      ...
      if (storedEnd) {
        ...
        logger.log("⏱️ Deriving end time from start time:", {
          startTime,
          totalSeconds,
          remaining,
        });
      ...
      lastToastTimeRef.current =
        Math.floor((minutes * 60 - remaining) / 300) * 300;
    } else {
      // Count-up timer
      ...
      if (isCountdownRef.current) {
        ...
        if (storedEnd) {
          ...
          if (remaining !== timeLeftRef.current) {
            ...
            const durationMatch = durationStr.match(/(\d+)/);
            const totalSecs =
              (durationMatch ? parseInt(durationMatch[1], 10) : 0) * 60;
            const elapsed = totalSecs - remaining;

            if (
              elapsed > 0 &&
              elapsed % 300 === 0 &&
              elapsed !== lastToastTimeRef.current
            ) {
  ...
  return (
    <div
      className={`flex items-center gap-2 px-4 py-2 rounded-lg font-mono text-lg font-semibold transition-all ${
        isCritical
