...
import { Play, Send, ChevronLeft, ChevronRight, Loader2, TerminalSquare } from 'lucide-react';
...
export default function CodingConsole({
    ...
    const [codeState, setCodeState] = useState<Record<number, { code: string, language: string, runOutput: string, errorOutput: string, results: any[], allPassed: boolean }>>(() => {
        try {
    ...
    useEffect(() => {
        ...
        if (codeState[currentQuestionIndex]) {
            ...
            const defaultCode = LANGUAGE_TEMPLATES[language] || `// Write your ${language} code here\n\n`;
            setCode(defaultCode);
            setRunOutput('');
            setErrorOutput('');
            setLastResults([]);
    ...
    useEffect(() => {
        setCodeState(prev => {
            const newState = {
                ...prev,
                [currentQuestionIndex]: { code, language, runOutput, errorOutput, results: lastResults, allPassed: lastAllPassed }
            };
            try {
                localStorage.setItem(`interview_code_${interviewId}`, JSON.stringify(newState));
            } catch (e) { }
            return newState;
        });
    }, [code, language, runOutput, errorOutput, lastResults, lastAllPassed, currentQuestionIndex, interviewId]);

    const handleLanguageChange = (newLang: string) => {
        setLanguage(newLang);
        setCode(LANGUAGE_TEMPLATES[newLang] || `// Write your ${newLang} code here\n\n`);
    };
    ...
    const runCode = async () => {
        ...
        try {
            ...
            if (response.data.success) {
                setRunOutput(response.data.output || 'Code executed successfully with no output.');
                ...
                setErrorOutput(response.data.message || response.data.stderr || 'Execution failed.');
            }
        } catch (err: any) {
            console.error('Execution failure:', err);
            setErrorOutput(err.response?.data?.message || err.message || 'Unknown execution error.');
        } finally {
            setIsRunning(false);
        }
    };
            ...
            <ResizablePanelGroup direction="horizontal" className="h-full w-full">
                ...
                <ResizablePanel defaultSize={40} minSize={30} className="bg-white flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto p-6 prose prose-slate max-w-none">
                        <h3 className="text-xl font-semibold mb-4 text-violet-900 border-b pb-2">
                            {currentQuestion?.title || `Coding Challenge ${currentQuestionIndex + 1}`}
                        </h3>
                        ...
                        {currentQuestion?.testCases && currentQuestion.testCases.length > 0 && !currentQuestion?.examples && (
                            <div className="mt-8">
                                <h4 className="font-bold text-slate-800 mb-3">Sample Test Cases:</h4>
                                {currentQuestion.testCases.map((tc, idx) => (
                                    <div key={idx} className="bg-slate-100 rounded-md p-4 mt-2 text-sm font-mono text-slate-800 whitespace-pre-wrap border border-slate-200">
                ...
                <ResizablePanel defaultSize={60} minSize={40}>
                    <ResizablePanelGroup direction="vertical" className="h-full">
                        ...
                        {/* Editor Area */}
                        <ResizablePanel defaultSize={65} minSize={20} className="flex flex-col bg-[#1e1e1e]">
                            {/* Editor Toolbar */}
                        ...
                        {/* Console/Output Area */}
                        <ResizablePanel defaultSize={35} minSize={15} className="bg-[#1e1e1e] flex flex-col">
                            <div className="px-4 py-2 bg-[#2d2d2d] border-b border-[#444] flex items-center justify-between flex-none">
                                ...
                                <Button
                                    variant="ghost"
                                    size="sm"
                                    onClick={() => { setRunOutput(''); setErrorOutput(''); }}
                            ...
                            <div className="flex-1 overflow-y-auto p-4 font-mono text-sm">
                                ...
                                {runOutput && !isRunning && (
                                    <pre className="text-green-400 whitespace-pre-wrap mb-4 font-mono">{runOutput}</pre>
                                )}
                                {errorOutput && !isRunning && (
                                    <pre className="text-red-400 whitespace-pre-wrap font-mono mt-2 bg-red-950/20 p-3 rounded-md border border-red-900/50">{errorOutput}</pre>
                                )}
                            </div>
                        </ResizablePanel>
