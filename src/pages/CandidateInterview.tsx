...
export default function CandidateInterview() {
    ...
    const submitInterview = async (history: Message[]) => {
        ...
        try {
            ...
            const res = await axios.post(`http://localhost:5000/api/interviews/feedback`, { conversation: history });
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
                            {Object.entries(feedback.rating).map(([key, score]: [string, any]) => (
                    ...
                    <CardFooter className="bg-muted/20 p-6 flex flex-col items-start gap-2">
                        <div className="font-semibold text-lg">Recommendation: <span className={feedback.Recommendation === 'Not Recommended' ? 'text-red-500' : 'text-green-600'}>{feedback.Recommendation}</span></div>
                        <p className="text-sm text-gray-500">{feedback["Recommendation Message"]}</p>
                    </CardFooter>
                </Card>
            </div>
        );
        ...
        <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
            <div className="w-full max-w-2xl space-y-6">
                <div className="text-center space-y-2">
                    ...
                    <p className="text-sm text-gray-500">Question {currentQuestionIndex + 1} of {interview.questions.length}</p>
                </div>
                ...
                <Card className="shadow-lg border-primary/10">
                    <CardHeader>
                        <span className="text-xs font-semibold text-violet-600 uppercase tracking-wider">{currentQ.type} Question</span>
                    ...
                    <CardFooter className="flex justify-between p-6 bg-gray-50/50">
                        ...
                        <Button size="lg" onClick={handleNext} disabled={isSubmitting} className="px-8">
                            {isSubmitting ? <Loader2 className="animate-spin mr-2" /> : (currentQuestionIndex === interview.questions.length - 1 ? "Submit Interview" : "Next Question")} <Send className="w-4 h-4 ml-2" />
                        </Button>
                    </CardFooter>
                </Card>
            </div>
