...
type SessionState = "loading" | "precheck" | "ready" | "interview" | "ending" | "error";
...
export default function InterviewSession() {
    ...
    const [transcriptHistory, setTranscriptHistory] = useState<{ role: string, content: string }[]>([]);
    ...
    useEffect(() => {
        const loadContext = () => {
            ...
            try {
                ...
                console.log("Context Loaded:", { name: userInfo.name, email: userInfo.email });
            } catch (e) {
    ...
    useEffect(() => {
        ...
        const handleBeforeUnload = (e: BeforeUnloadEvent) => {
            e.preventDefault();
            e.returnValue = "Are you sure you want to leave? Your interview is in progress.";
    ...
    }, [id, sessionState, transcriptHistory, elapsedTime, violations, tabSwitchCount, backNavAttempts]);
    ...
    const handleViolation = useCallback((type: string, description: string) => {
        setViolations(prev => [...prev, { type, description, timestamp: new Date().toISOString() }]);
    }, []);

    const handleFaceStatus = useCallback((isDetected: boolean) => {
    ...
    const setupRetellListeners = () => {
        ...
        client.on("call_started", () => {
            setIsListening(true);
            toast({ title: "Interview Started", description: "Good luck! Speak clearly." });
        });
        client.on("call_ended", () => handleEndInterview("Completed"));
        client.on("agent_start_talking", () => { setIsSpeaking(true); setIsListening(false); });
        client.on("agent_stop_talking", () => { setIsSpeaking(false); setIsListening(true); });
        ...
        client.on("error", (err) => {
            ...
            toast({ title: "Connection Unstable", description: "Please check your internet.", variant: "destructive" });
        });
    };
    ...
    const startInterview = async () => {
        ...
        try {
            ...
            const finalName = candidateName || interviewData?.candidateName || candidateEmail.split('@')[0];
            ...
            const { data } = await axios.post(`${API_URL}/retell/create-web-call`, {
                retell_llm_dynamic_variables: {
                    ...
                    job_description: interviewData?.jobDescription || "Standard interview."
                }
            });
            ...
            toast({ title: "Start Failed", description: "Could not initialize AI agent.", variant: "destructive" });
            setSessionState("precheck");
        }
    };
    ...
    const handleEndInterview = async (finalStatus: string, reason?: string) => {
        ...
        try {
            await axios.post(`${API_URL}/interviews/results`, {
                ...
                candidate_name: candidateName || interviewData?.candidateName || candidateEmail,
                scores: { "Total": 0 },
    ...
    if (sessionState === "loading") return <LoadingScreen message="Loading interview session..." />;
    if (sessionState === "error") return <ErrorScreen onBack={() => navigate("/candidate/dashboard")} />;
    if (sessionState === "precheck") return (
        <PrecheckScreen
    ...
    if (sessionState === "ready" || sessionState === "ending") return <LoadingScreen message={sessionState === 'ready' ? "Connecting to AI..." : "Finalizing session..."} />;
            ...
            <header className="border-b border-white/10 bg-black/50 backdrop-blur-md px-6 py-4 sticky top-0 z-50">
                <div className="max-w-screen-2xl mx-auto flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="font-bold text-lg leading-tight tracking-tight">{interviewData?.jobRole}</h2>
                            <div className="flex items-center gap-2 mt-0.5">
                                <span className="flex h-2 w-2 rounded-full bg-emerald-500 animate-pulse"></span>
                                <p className="text-xs text-white/50 font-medium">Live Session Active</p>
                            </div>
                        </div>
                        {(tabSwitchCount + backNavAttempts > 0) && (
                            <Badge variant="destructive" className="ml-4 flex gap-1 items-center animate-in fade-in zoom-in">
                    ...
                    <div className="flex items-center gap-6">
                        <div className="flex flex-col items-end">
                            <p className="text-[10px] uppercase font-bold tracking-widest text-white/40 mb-1">Duration</p>
                            <Badge variant="outline" className="font-mono text-primary border-primary/20 bg-primary/5 px-3 py-1">
                    ...
                    <Card className="flex-[2] bg-gradient-to-b from-[#111] to-[#0a0a0a] border-white/10 relative overflow-hidden flex items-center justify-center shadow-2xl">
                        ...
                        <div className={cn(
                            ...
                            <Bot className={cn(
                                ...
                                isSpeaking ? "text-primary drop-shadow-[0_0_20px_rgba(124,58,237,0.5)]" : "text-white/20"
                            )} />
                        </div>
                            ...
                            {transcriptHistory.map((t, i) => (
                                <div key={i} className={cn(
                                    ...
                                    <p className="text-[9px] uppercase font-bold tracking-wider mb-1.5 opacity-50 flex items-center gap-1">
                                        {t.role === 'agent' ? <Bot className="h-3 w-3" /> : <User className="h-3 w-3" />}
                ...
                <AlertDialogContent className="bg-[#111] border-white/10 text-white">
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription className="text-white/60">
                            This will submit your responses and conclude the session. This action cannot be undone.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel className="bg-white/5 border-white/10 text-white hover:bg-white/10">Resume Protocol</AlertDialogCancel>
                        <AlertDialogAction onClick={() => handleEndInterview("Completed")} className="bg-red-600 hover:bg-red-700 text-white border-0">Conclude Session</AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
            </AlertDialog>
                ...
                <AlertDialogContent className="bg-red-950/20 border-red-500/20 text-white backdrop-blur-xl">
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription className="text-white/80">
                            Navigation attempt detected. This action is flagged as a potential security violation.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel onClick={() => setShowBackNavDialog(false)} className="bg-white/5 border-white/10 text-white">Return to Session</AlertDialogCancel>
                        <AlertDialogAction onClick={() => handleEndInterview("Terminated", "User attempted back navigation")} className="bg-red-600 hover:bg-red-700">Terminate Session</AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
            </AlertDialog>
        </div>
...
function LoadingScreen({ message }: { message: string }) {
    return (
        <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center gap-4">
            ...
            <p className="text-white/50 animate-pulse font-medium tracking-wide">{message}</p>
        </div>
    );
}
            ...
            <Card className="max-w-md w-full bg-[#111] border-white/10 shadow-2xl">
                <CardContent className="p-8 text-center space-y-6">
                    ...
                    <div>
                        ...
                        <p className="text-white/50">This interview session cannot be accessed. It may be invalid, expired, or already completed.</p>
                    </div>
                    <Button onClick={onBack} size="lg" className="w-full font-semibold">Return to Dashboard</Button>
                </CardContent>
            </Card>
        </div>
    );
...
function PrecheckScreen({ interviewData, candidateName, candidateEmail, onStart, onGrant, permissionsGranted, onCancel }: any) {
            ...
            <Card className="w-full max-w-4xl bg-[#0f0f0f] border-white/10 shadow-2xl overflow-hidden relative z-10 grid md:grid-cols-5">
                ...
                <div className="md:col-span-2 bg-white/5 p-8 flex flex-col justify-between border-r border-white/5">
                    <div>
                        ...
                        <h1 className="text-2xl font-bold text-white mb-2 leading-tight">Technical<br />Interview</h1>
                        <p className="text-white/40 text-sm mb-6">AI-Powered Assessment</p>

                        <div className="space-y-4">
                            <div className="p-3 rounded-lg bg-black/40 border border-white/5">
                                <p className="text-[10px] uppercase font-bold text-white/30 mb-1">Position</p>
                                <p className="text-white font-medium">{interviewData?.jobRole}</p>
                            </div>
                            <div className="p-3 rounded-lg bg-black/40 border border-white/5">
                                <p className="text-[10px] uppercase font-bold text-white/30 mb-1">Candidate</p>
                                <div className="flex items-center gap-2">
                                    <User className="h-3.5 w-3.5 text-primary" />
                                    <p className="text-white font-medium text-sm truncate">{candidateName}</p>
                                </div>
                                <div className="flex items-center gap-2 mt-1">
                                    <Mail className="h-3.5 w-3.5 text-white/40" />
                                    <p className="text-white/40 text-xs truncate">{candidateEmail}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="md:col-span-3 p-8 flex flex-col justify-center">
                    <div className="mb-8 space-y-2">
                        ...
                        <p className="text-white/50 text-sm">Review protocols before beginning.</p>
                    </div>
                    ...
                    <div className="space-y-4 mb-8">
                        <div className="flex gap-4 p-4 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-colors">
                            ...
                            <div>
                                <h3 className="font-medium text-white text-sm">Proctoring Active</h3>
                                <p className="text-xs text-white/40 leading-relaxed mt-1">
                                    Full-screen monitoring enabled. Tab switching or face absence will be flagged as violations.
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex gap-4 p-4 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-colors">
                            ...
                            <div>
                                <h3 className="font-medium text-white text-sm">Duration: {interviewData?.duration} Minutes</h3>
                                <p className="text-xs text-white/40 leading-relaxed mt-1">
                    ...
                    <div className="flex flex-col gap-3">
                        {!permissionsGranted ? (
                            <Button onClick={requestPermissions} size="lg" className="w-full text-base font-semibold h-14 bg-white/10 hover:bg-white/20 text-white border-0">
                                <CheckCircle className="mr-2 h-5 w-5" />
                                Verify Camera & Audio
                            </Button>
                        ) : (
                            <Button onClick={onStart} size="lg" className="w-full text-base font-bold h-14 bg-gradient-to-r from-primary to-violet-600 hover:opacity-90 shadow-lg shadow-primary/25 border-0 animate-in fade-in zoom-in">
                                Initialize Secure Session
                            </Button>
                        )}
                        <Button variant="ghost" onClick={onCancel} className="text-white/30 hover:text-white hover:bg-transparent">
                            Cancel Session
                        </Button>
                    </div>
                </div>
