...
import { Video, Play, History, Star, ArrowRight, Zap, Loader2, Upload } from "lucide-react";
...
export default function CandidateMock() {
    ...
    const fetchRecentSessions = async () => {
        try {
            setIsLoading(true);
            const { data } = await axios.get(`${API_URL}/mock-interviews/history/${getUserEmail()}`);
    ...
    const createMockInterview = async (type: 'general' | 'tailored') => {
        try {
            ...
            const payload = {
                ...
                jobDescription: type === 'tailored' ? jobDescription : 'Standard industry questions for common roles.',
                interviewType: 'Mock',
                duration: 30
            };

            const { data } = await axios.post(`${API_URL}/mock-interviews/create`, payload);
            ...
            toast({
                title: "Error",
                description: error.response?.data?.message || "Failed to create mock interview",
                variant: "destructive"
            });
        } finally {
            setIsCreating(false);
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            <div className="flex flex-col gap-2">
                ...
                <p className="text-muted-foreground">Practice with our AI agent in a realistic interview environment.</p>
            </div>
            ...
            <div className="grid gap-6 md:grid-cols-3">
                ...
                <Card className="md:col-span-2">
                    <CardHeader>
                        ...
                        <CardDescription>Select a role or upload a job description to begin a tailored practice session.</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-6">
                        <div className="grid gap-4 sm:grid-cols-2">
                            ...
                            >
                                ...
                                <p className="text-sm text-muted-foreground mb-4">Focused on your specific target job description.</p>
                                <Button
                                    className="w-full"
                                    disabled={isCreating}
                                    onClick={(e) => {
                                ...
                                >
                                    {isCreating ? <Loader2 className="h-4 w-4 animate-spin" /> : <>Start <Play className="ml-2 h-4 w-4" /></>}
                                </Button>
                            </div>
                            <div
                            ...
                            >
                                ...
                                <p className="text-sm text-muted-foreground mb-4">Standard industry questions for common roles.</p>
                                <Button
                                    variant="outline"
                                    className="w-full"
                                    disabled={isCreating}
                                    ...
                                    {recentSessions.slice(0, 5).map((session, i) => (
                                        ...
                                        >
                                            <div className="flex-1 min-w-0">
                                                <h5 className="text-sm font-semibold truncate">{session.jobRole}</h5>
                                                <p className="text-xs text-muted-foreground">
                                                    {formatDate(session.completedAt)} • {session.duration}m
                                                </p>
                                            </div>
                                    ...
                                    <div className="p-4">
                                        <Button
                                            ...
                                            onClick={() => {
                                                toast({
                                                    title: "Coming Soon",
                                                    description: "Full session history view is under development",
                                                });
                                            }}
                                        >
            ...
            <Dialog open={showTailoredDialog} onOpenChange={setShowTailoredDialog}>
                <DialogContent className="sm:max-w-[500px]">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Provide details about the role you're preparing for to get customized interview questions.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-4">
                        ...
                        <div className="space-y-2">
                            ...
                            />
                            <p className="text-xs text-muted-foreground">
                                The more details you provide, the more tailored your practice session will be.
                            </p>
                        </div>
                    </div>
