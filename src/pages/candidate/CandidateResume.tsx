...
import { FileText, Upload, CheckCircle2, AlertCircle, Sparkles, TrendingUp, Target, Lightbulb, History, Clock, FileEdit, Video, Copy, Loader2 } from "lucide-react";
...
export default function CandidateResume() {
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                const historyRes = await axios.get(`${API_URL}/resume/optimize/history`, config);
    ...
    const handleGenerateCoverLetter = async () => {
        ...
        try {
            const { data } = await axios.post(`${API_URL}/resume/cover-letter`, {
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <p className="text-xs text-muted-foreground">Paste a job description to score against it for a specific role.</p>
                                <Textarea
                    ...
                    {history.length > 0 && !analysis && (
                        <Card className="animate-in fade-in slide-in-from-bottom-4">
                            <CardHeader>
                                ...
                                <CardDescription>Review your past resume optimization results.</CardDescription>
                            </CardHeader>
                            <CardContent>
                            ...
                            <CardContent className="space-y-8">
                                ...
                                {analysis.bulletPointRewrites && analysis.bulletPointRewrites.length > 0 && (
                                    <div className="space-y-4 pt-4 border-t border-border">
                                        ...
                                        <p className="text-sm text-muted-foreground">Upgrade weak bullet points using our AI suggestions.</p>
                                        <div className="space-y-4">
                                            {analysis.bulletPointRewrites.map((rewrite, i) => (
                                                <div key={i} className="grid md:grid-cols-2 gap-4">
                                                    <div className="p-4 bg-destructive/5 rounded-xl border border-destructive/20 relative">
                                                        <div className="absolute top-2 right-2 text-[10px] font-bold uppercase text-destructive/70 bg-destructive/10 px-2 py-0.5 rounded">Original</div>
                                                        <p className="text-sm text-muted-foreground italic mt-2">"{rewrite.original}"</p>
                                                    </div>
                                                    <div className="p-4 bg-success/5 rounded-xl border border-success/20 relative">
                                                        <div className="absolute top-2 right-2 text-[10px] font-bold uppercase text-success bg-success/10 px-2 py-0.5 rounded">Better</div>
                                                        <p className="text-sm font-medium mt-2">"{rewrite.rewrite}"</p>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                                ...
                                <div className="grid md:grid-cols-2 gap-4 pt-4 border-t border-border">
                                    ...
                                    >
                                        {isGeneratingCoverLetter ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <FileEdit className="w-4 h-4 mr-2" />}
            ...
            <Dialog open={showCoverLetterDialog} onOpenChange={setShowCoverLetterDialog}>
                <DialogContent className="sm:max-w-[700px] h-[80vh] flex flex-col">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Tailored specifically to your strengths and the target job description.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <DialogFooter className="mt-4 flex sm:justify-between items-center w-full">
                        <Button variant="outline" onClick={() => setShowCoverLetterDialog(false)}>Close</Button>
                        <Button onClick={() => {
