      ...
      if (data.success && data.data) {
        ...
        if (request.status === "Action Required") {
          ...
          toast({
            ...
            description:
              "Your reschedule request is still being reviewed by HR.",
          });
        } else {
          toast({
            title: "Status Update",
    ...
    <div className="space-y-8 animate-fade-in">
      <div className="flex flex-col gap-2">
        <h1 className="text-3xl font-bold tracking-tight">
          Welcome back, {userInfo.name || "Candidate"}!
        </h1>
        <p className="text-muted-foreground">
          Here's what's happening with your job applications and AI career
          growth.
        </p>
      </div>
        ...
        <Card className="hover:shadow-md transition-shadow">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {latestAnalysis ? `${latestAnalysis.atsScore}%` : "0%"}
            </div>
            <p className="text-xs text-muted-foreground mt-1 text-success flex items-center gap-1">
              {latestAnalysis ? (
                <>
                  <TrendingUp className="h-3 w-3" /> Profile Analyzed
                </>
              ) : (
                <>
                  <TrendingUp className="h-3 w-3" /> New Candidate
                </>
              )}
            </p>
          </CardContent>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Official recruitment interviews waiting for you
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                ...
                <div className="space-y-4">
                  {interviews.map((interview) => {
                    const scheduledDate = interview.scheduledDate
                      ...
                      : null;
                    const isTooEarly = scheduledDate
                      ? isBefore(new Date(), subMinutes(scheduledDate, 30))
                      : false;

                    return (
                      <div
                        key={interview._id}
                        className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border bg-muted/30 gap-4"
                      >
                          ...
                          <div className="space-y-1">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              {interview.status === "Rescheduled" && (
                                <Badge
                                  variant="outline"
                                  className="text-[10px] h-4 border-primary/20 bg-primary/10 text-primary"
                                >
                                  Rescheduled
                                </Badge>
                              )}
                              {interview.status === "Active" &&
                                scheduledDate && (
                                  <Badge
                                    variant="outline"
                                    className="text-[10px] h-4 border-success/20 bg-success/10 text-success"
                                  >
                                    Confirmed
                                  </Badge>
                                )}
                            ...
                            <p className="text-xs text-muted-foreground">
                              {interview.interviewType} Assessment
                            </p>
                            {scheduledDate && (
                        ...
                        <div className="flex gap-2 self-end sm:self-center">
                          ...
                          <Button
                            size="sm"
                            disabled={
                              isTooEarly || interview.status === "Rescheduled"
                            }
                            title={
                              isTooEarly
                                ? `Interview will be available 30 mins before ${format(scheduledDate!, "p")}`
                                : ""
                            }
                            onClick={() => {
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                <div className="space-y-1 z-10">
                  ...
                  <p className="text-sm text-blue-700/80">
                    Launch an on-demand AI mock session to sharpen your speaking
                    skills and reduce pre-interview anxiety.
                  </p>
                </div>
                <Button
                  className="shrink-0 bg-blue-600 hover:bg-blue-700 z-10"
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Areas discovered from your resume
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {latestAnalysis ? (
                <>
                  {latestAnalysis.areasForImprovement
                    ?.slice(0, 3)
                ...
                {latestAnalysis ? (
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-xs font-medium">
                      Resume Score: {latestAnalysis.atsScore}%
                    </p>
                  </div>
                ) : (
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-xs font-medium">
                      Upload Resume to unlock achievements
                    </p>
                  </div>
                )}
                <div className="flex items-center gap-3">
